...
session_start();
        ...
        if (strlen($newimg1) > 1) {
            $sql="UPDATE tb_users SET picture_profile='$newimg1', name='$name', email='$email', no_tlp='$no_tlp', password='$password'  WHERE id_user='$id_user'";
        } else {
            $sql="UPDATE tb_users SET name='$name', email='$email', no_tlp='$no_tlp', password='$password'  WHERE id_user='$id_user'";
        }
